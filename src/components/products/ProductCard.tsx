import Link from "next/link";
import Image from "next/image";
import { IProduct } from "@/types";
import { WHATSAPP_URL } from "@/lib/constants";

export default function ProductCard({ product }: { product: IProduct }) {
  const hasImage = product.images.length > 0;
  const whatsappMsg = encodeURIComponent(
    `Hello EcoGlow! I'm interested in ${product.name}${product.subtitle ? ` (${product.subtitle})` : ""}. Please share more details.`
  );

  return (
    <div className="group bg-white rounded-xl border border-border overflow-hidden hover:shadow-lg transition-all duration-200">
      <Link href={`/products/${product.slug}`} className="block">
        <div className="relative aspect-[4/3] bg-gray-50 overflow-hidden">
          {hasImage ? (
            <Image
              src={product.images[0]}
              alt={product.name}
              fill
              className="object-cover group-hover:scale-105 transition-transform duration-300"
              sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw"
            />
          ) : (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-brand-teal/10 to-brand-leaf/10">
              <svg
                width={48}
                height={48}
                viewBox="0 0 100 120"
                fill="none"
                className="opacity-30 mb-2"
              >
                <path
                  d="M50 8 C50 8, 15 55, 15 72 C15 92, 30 108, 50 108 C70 108, 85 92, 85 72 C85 55, 50 8, 50 8Z"
                  fill="#00897B"
                />
              </svg>
              <span className="text-sm text-muted">Image coming soon</span>
            </div>
          )}
          {product.category && (
            <span
              className="absolute top-3 left-3 px-2.5 py-1 text-xs font-medium text-white rounded-full"
              style={{ backgroundColor: product.category.accentColor }}
            >
              {product.brandLine}
            </span>
          )}
        </div>
      </Link>
      <div className="p-4">
        <Link href={`/products/${product.slug}`}>
          <h3 className="font-bold text-brand-dark-green font-[family-name:var(--font-heading)] mb-1 group-hover:text-brand-teal transition-colors">
            {product.name}
          </h3>
          {product.subtitle && (
            <p className="text-sm text-muted mb-2">{product.subtitle}</p>
          )}
          <p className="text-sm text-foreground/70 line-clamp-2 mb-3">
            {product.purpose}
          </p>
        </Link>
        <div className="flex gap-2">
          <Link
            href={`/products/${product.slug}`}
            className="flex-1 text-center px-3 py-2 text-sm font-medium text-brand-teal border border-brand-teal rounded-lg hover:bg-brand-teal hover:text-white transition-colors"
          >
            View Details
          </Link>
          <a
            href={`${WHATSAPP_URL}?text=${whatsappMsg}`}
            target="_blank"
            rel="noopener noreferrer"
            className="px-3 py-2 bg-[#25D366] text-white rounded-lg hover:bg-[#20BD5A] transition-colors"
            aria-label={`Order ${product.name} via WhatsApp`}
          >
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
            </svg>
          </a>
        </div>
      </div>
    </div>
  );
}
